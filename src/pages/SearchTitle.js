    ...
    const findTitle = async () => {
      try {
        let res = await moviesdb.get(`${type}/${match.params.id}`, {
          params: { api_key: "c625140e19067452daf7cd175f3fe6be" },
        });
                      ...
                      {isTv ? (
                        <h3
                          className={`show__production show__production--${
                            title.in_production ? "Transmitiendo" : "Completada"
                          }`}
                        >
                          {title.in_production ? "En Produccion" : "Completada"}
                        </h3>
                      ) : null}
                    </div>
                    <div className="show__votes">
                      <h3 className="show__vote-average">{`${title.vote_average}/10`}</h3>
                      <h3 className="show__vote-count">{`${title.vote_count} votos`}</h3>
                    </div>
                    {title.tagline ? (
                      <h2 className="show__tagline">{title.tagline}</h2>
                    ) : null}
                    {title.number_of_episodes ? (
                      <h2 className="show__seasons">{`${title.number_of_seasons} Temporada(s) ${title.number_of_episodes} Episodios`}</h2>
                    ) : null}
